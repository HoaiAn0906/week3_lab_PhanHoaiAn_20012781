...
public class SkillService {
    ...
    public DataResponse<Page<Skill>> getAllSkills(String filterQ, int page, int limit, String sortBy, String sortDirection) {
        ...
        if (filterQ != null && !filterQ.isEmpty()) {
            skillsPage = skillRepository.findSkillsBySkillNameContainingIgnoreCase(filterQ, pageable);
        } else {
            skillsPage = skillRepository.findAll(pageable);
        }

        return DataResponse.<Page<Skill>>builder()
                .data(skillsPage)
                .status(200)
                .message("Success")
                .build();
